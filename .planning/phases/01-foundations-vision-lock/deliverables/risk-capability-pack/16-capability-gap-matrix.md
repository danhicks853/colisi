...
|-------|---------------|:--:|:--:|:--:|------------------|----------|--------|
| **GDScript syntax + idiom** | Phase 3 | {Daniel scores} | 4 | {computed} | Godot docs tutorial sequence + GDQuest curriculum (per `STACK.md` Recommended Stack) | End of Phase 2 (~m4) | Open |
| **Godot 4.5.x scene composition** | Phase 3 | {Daniel scores} | 4 | {computed} | Heartbeast / GDQuest 3D-cozy tutorials | Phase 3 | Open |
| **Blender 3D modeling** | Phase 6 | {Daniel scores} | {Daniel scores — target depends on art-source decision per `INFRA-08`} | {computed} | YouTube curriculum + indie-cozy character tutorials; Blender LTS pinned | Phase 5 | Open |
| **Blender rigging + glTF export (`STACK.md` 3D Art Pipeline)** | Phase 6 | {Daniel scores} | 3 | {computed} | Blender Studio glTF guide + supermatrix.studio Blender→Godot guide; NLA Strips workflow | Phase 5 | Open |
| **Mixamo + retargeting workflow** | Phase 6 | {Daniel scores} | 3 | {computed} | Asset Library Mixamo Retargeter README + manual bone-map fallback | Phase 5–6 | Open |
| **Dialogue Manager (Nathan Hoad)** | Phase 4 | {Daniel scores} | 3 | {computed} | Dialogue Manager docs + example projects; pin to specific version (`STACK.md`) | Phase 3 | Open |
| **Custom Resource save schema + migrations** | Phase 3 | {Daniel scores} | 4 | {computed} | GDQuest save-game-godot4 article + practice migration; atomic save-write protocol (`STACK.md`) | Phase 3 | Open |
| **Steam Web API (for `MKTG-06` review mining)** | Phase 1 | {Daniel scores} | 3 | {computed} | One-page tutorial; `appreviews` endpoint; LLM helps wrap calls per `D-MKTG-02` | Phase 1 execution (plan 01-07) | Open |
| **Google Forms + Sheets analysis (for `MKTG-06`)** | Phase 1 | {Daniel scores} | 3 | {computed} | Standard usage; LLM helps with formulas | Phase 1 execution (plan 01-07) | Open |
...
| **Music production (Bosca Ceoil → LMMS)** | Phase 6+ | {Daniel scores} | {Daniel scores — target depends on composer-hire decision per `STACK.md`} | {computed} | LMMS + Bosca tutorials (`STACK.md` Audio); revisit composer-hire decision post-revenue | Phase 6 | Open |
| **Narrative writing for state-tagged dialogue** | Phase 4+ | {Daniel scores} | {Daniel scores} | {computed} | Inkle "variables not branches" (`PHILOSOPHY.md` §I.4) + Disco Elysium voice corpus + Daniel's own writing practice | Ongoing | Open |
...
| **Solo-dev project management discipline** | All phases | {Daniel scores} | 4 | {computed} | This GSD workflow; monthly retros; Year-3-Wall check-ins (`HEALTH-03..05`) | Ongoing | Open |
| **GUT 9.x test authoring (save/load, calendar, dialogue conditions)** | Phase 3+ | {Daniel scores} | 3 | {computed} | GUT documentation (`STACK.md` Testing Approach); test only systems that hurt to break | Phase 3 | Open |
| **Git LFS configuration + maintenance** | Phase 2 | {Daniel scores} | 3 | {computed} | `STACK.md` Version Control + Git LFS Strategy; INFRA-01 deliverable | Phase 2 | Open |
