...
- **D-VISION-04:** **Two-layer puzzle structure with distinct rhythms (sharpened 2026-05-10).** Both layers are central; their rhythms differ.

  - *Moment-to-moment layer (frequent):* magic-tutoring through per-villager skill minigames — the sprite helps each villager surface and exercise their personal magic via short, atomic interactions tied to their craft (e.g., cooking-magic minigame with a baker, rhythm-spell at a lathe with a carpenter, etc.). This is the daily-loop activity; each villager has a minigame surface that grows with relationship depth. Magic-tutoring is the operationalization of D-VISION-02's "latent magic surfacing through relationship."

  - *Meta puzzle layer (rare, high-meaning):* **matchmaking-via-environmental-manipulation** — story-anchor arcs in which the sprite observes villagers, schedules, items, dialogue cues; identifies pairing opportunities; gently manipulates environment (a falling petal, a redirected breeze, a kettle whistling a moment too long) to spark a meet-cute. Some encounters become friendships, some become love, some never spark — all are valid outcomes. Solution discovery is observation-based; satisfies NARR-07 (puzzles must telegraph).

  Pattern reference: Hades-shape (frequent combat run + meta dialogue/relationship arcs between runs) and Stardew-shape (daily activity loop + seasonal heart-events / festival beats). The original 2026-05-09 lock ("central puzzle layer is matchmaking-via-environmental-manipulation") understated the moment-to-moment magic-tutoring activity; this 2026-05-10 sharpening adds the layer above without contradicting the matchmaking puzzle layer below. Downstream consumers (05-core-loops.md, 07-pillar-mechanic-matrix.md, 23-why-play-at-hour-200.md) must distinguish the two rhythms explicitly when deriving from D-VISION-04.
...
- **D-VISION-09:** **NG+ via wisp's accumulated cross-playthrough memory (time-loop framing).** [Locked 2026-05-10 during 06-player-journey.md Hour 100 brainstorm.] At completion, player can begin a new save (NG+) where the wisp carries event-log knowledge from prior playthroughs forward. Same village, same 12-15 NPCs, same world — narration overlay + UI affordances are the only new content (no new villager / world / asset production). Wisp surfaces archetypal recall-narration at key inflection points; previously-chosen dialog options get UI-highlight. **Time-loop diegetic framing** locked over "wisp visits another village" framing for budget-tightness and thematic coherence. **Five tractability constraints locked together:** (1) NG+ cap at 3 runs total; (2) coarse archetypal signatures (~20-30 archetypes) for wisp narration + per-event UI highlight for dialog-choice memory; (3) ~20-50 archetypal trigger points per playthrough (≈1-2 per hour at slice density); (4) open-ended narration, not branching forks (wisp comments, player still chooses freely); (5) time-loop diegetic frame. **NG+ never overwrites — always its own save file.** Architectural amendments queued for SAVE-04 (multi-playthrough event-log persistence) and NARR-06 (wisp-recall narration variants + archetypal event tagging + dialog-highlight UI affordance). Cozy-promise compliance: doesn't violate restart-to-rebranch anti-clause (NG+ is a new save, original preserved untouched); pairs with cascading thread + state-tagged dialogue depth + seasonal recurrence + replay-as-revelation as the meta-loop's hour-100+ engagement layer. Pillar 1 (character development) deepened (wisp itself grows); Pillar 2 (never punishing) honored (memory is gift, not difficulty); Pillar 3 (approachability) **requires deliberate NG+ re-introduction tutorial design** — NG+ technically slides past the "mid-game introduction of untaught mechanics" anti-clause because it's a NEW save, but only if the new-game framing + re-introduction tutorial land cleanly. **NG+ re-introduction tutorial is a Phase 4 (dialogue/NARR scripting for the wisp's first NG+ session) + Phase 6 (full-game content authoring for the tutorial archetypal beats) design item; full-game scope, not slice scope** (slice ends at hour 10 cliffhanger; NG+ never fires in slice). Cross-references: SCOPE.md row 6 (2026-05-10), `06-player-journey.md` Hour 100 + Hour 500 rows, `23-why-play-at-hour-200.md` (plan 01-06), Phase 4 + Phase 6 plans (NG+ tutorial design TODO).
- **D-VISION-10:** **HOME cluster full transmutation under sprite premise (3-layer architecture).** [Locked 2026-05-10 during 07-pillar-mechanic-matrix.md HOME-cluster reinterpretation discussion.] Three layers compose the wisp's relationship to space:
  - **Layer 1 — Starting sanctum:** default rest location pre-completion; provides D-TIME-03 rest trigger before any villager arcs are complete; final form locked at plan 01-05 World Bible (tree hollow / glade / abandoned shrine candidates).
  - **Layer 2 — Village-as-canvas:** milestone-driven shared-space transformation with wisp-as-chooser. Villagers initiate permanent transformations of shared village spaces (town square, central tree, festival grounds, well, etc.) in honor of / thanks to the wisp at villager-arc milestones AND village-wide milestones; player picks from 2-4 authored alternatives per upgrade slot ("fountain or lawn with benches?"); ~50+ upgrade slots full game; **2 slots in slice**; defer/decline always valid (Pillar 2 honored — choice persists indefinitely; villagers patiently wait); one-way within playthrough; replayability via D-VISION-09 NG+ differentiation (each NG+ run can produce different village aesthetic accumulation); wisp NG+ memory affects choice-presentation narration + dialog-highlight UI per D-VISION-09 integration. NPC commentary depth: triggering villager full reactive commentary; other villagers generic "the village looks different now" responses.
  - **Layer 3 — Faerie houses (Dawn's contribution 2026-05-10):** when wisp fully completes a villager's arc, that villager places a faerie house for the wisp; instanced area, NOT customizable (preserves wisp-as-guest framing; reduces customization burden); themed and decorated based on the villager who placed it (each faerie house is a Pillar 1 crystallized character relationship); 12-15 unique villager-themed designs full game; **1 design in slice** (whichever single villager arc completes within slice scope, if any); after unlocking, any faerie house can serve as a rest location alongside starting sanctum (D-TIME-03 rest-trigger flexibility).
  Cross-system integration: NARR-06 replay system (each milestone choice = event-log significance marker), SAVE-04 amendment grows further (per-zone aesthetic state + per-villager faerie-house unlock state, multi-playthrough-persistent under D-VISION-09), Phase 6 content load (~100-200 aesthetic variations + 12-15 faerie house designs, all character-tied + bounded). REQUIREMENTS.md HOME-01..05 amendments queued for Phase 1→2 transition. Cozy-promise compliance: every aesthetic is EARNED through character work (anti-filler), defer/decline always valid (anti-pressure), wisp doesn't decorate directly (preserves wisp-as-guest framing), no fail state. Cross-references: SCOPE.md row 7 (2026-05-10), D-VISION-04 (cascading thread — visible village changes are the seeds for next anchor), D-VISION-09 (NG+ differentiation), `02-pillars-of-play.md` Pillar 1, `07-pillar-mechanic-matrix.md` HOME rows re-scored, `12-world-bible-v0.1.md` (plan 01-05 — village zones authored with upgrade-slot density).
- **D-VISION-11:** **Stray black cat + animal rescue meta-loop anchor type.** [Locked 2026-05-10 during 07-pillar-mechanic-matrix.md NPC-06 reinterpretation discussion; honors Dawn's pet-adoption emergent-feedback entry 2026-05-10.] Discoverable optional companion: the wisp can find a stray black cat (or not — replayability via discovery; not mandatory content). Two integration points using existing mechanics (no new systems to learn): **(1)** Magic-tutoring minigames imbue the cat with magic abilities (speech, flying, humorous magical cat mannerisms — final form Daniel-authored at Phase 6 content authoring). **(2)** Matchmaking system finds the cat an owner among villagers. **Animal rescue** added to meta-loop anchor-type catalog (alongside matchmaking, personal-magic mastery, village-wide projects, individual character arcs, friendship/found-family arcs, seasonal recurrence) — black cat is the **tutorial case** for the animal rescue anchor type; gentle introduction via single-cat case before broader animal-rescue content lands in full-game development. **Slice scope:** cat appears as discoverable optional content in slice; cat arc does NOT complete in slice (slow-burn thread that drives wishlist hook — "I want to know what happens to the cat"). **NPC-06 reinterpretation:** the 12-15 villager cast includes multiple non-human characters (talking animals, spirits, witches consistent with magical-realism setting); the stray cat is OPTIONAL DISCOVERABLE companion separate from the cast. NPC-06 REQUIREMENTS.md wording amendment queued for Phase 1→2 transition: from "at least one NPC is a non-human companion" to "multiple villagers in the cast are non-human, plus optional discoverable non-villager companions (e.g., the stray black cat)." Cozy-promise compliance: cat is optional (no fail state for missing it), uses only locked mechanics (no untaught mid-game system per Pillar 3 anti-clause), tutorial introduction is gentle. Cross-references: SCOPE.md row 8 (2026-05-10), D-VISION-04 (animal rescue is a meta-loop anchor type), `persona-zero-emergent-feedback.md` pet-adoption entry status moves candidate → adopted, `07-pillar-mechanic-matrix.md` NPC-06 row re-scored from 2,0,0 to 3,0,0, `05-core-loops.md` meta-loop anchor catalog adds animal-rescue type, `09-slice-scenario-sketch.md` (plan 01-04 — cat as discoverable slice content with cliffhanger).
- **D-VISION-12:** **Spritesight mechanic — gift-based per-villager awareness gating direct interaction.** [Locked 2026-05-10 during slice cast Q4 first-sighted-villager discussion.] Diegetic solution to "how can the wisp converse with villagers who can't see it." **Cyrus has spritesight from the start** (via psychic knowledge + blindness archetype combination — the blind seer trope). Other villagers can perceive the wisp only AFTER receiving the spritesight gift from Cyrus. **Pre-spritesight phase:** wisp observes the villager passively (drift through their space, watch them work, learn habits / interests / arc-state); cannot direct-dialog them. Wisp can gently manipulate the environment to nudge the villager toward visiting Cyrus where the spritesight gift is bestowed. **Post-spritesight:** direct dialog opens (NPC-05 state-tagged + NARR-01 branching); magic-tutoring minigame becomes available with this villager; matchmaking-eligibility unlocks. **Slice scope:** all 3 of Sprinkles / Ari / Zhan receive spritesight in slice (Cyrus sighted from start = 4 total spritesight beats in slice). **Architectural implications:** per-villager `is_sighted: bool` field (SAVE-04 event log); pre-sighted vs sighted dialog state branching (NPC-05 amendment queued); new mechanic "wisp guides villager to Cyrus" via environmental manipulation (Phase 4/6 design); Cyrus's spritesight-bestowal action is recurring beat (~12-15 times full game). **Cozy-promise compliance:** fully diegetic (Pillar 3 no-untaught-mid-game-system honored — mechanic is gentle, telegraphed via Cyrus's gift); pre-spritesight observation IS positive play content, not gating obstacle. **Replayability:** different spritesight-introduction orders across playthroughs produce different first-contact experiences (slice replayability hook). Cross-references: SCOPE.md row 11 (2026-05-10), `09-slice-scenario-sketch.md` (plan 01-04 — slice authors 4 spritesight beats), `12-world-bible-v0.1.md` (plan 01-05 — spritesight lore and how Cyrus learned it), `23-why-play-at-hour-200.md` (plan 01-06 — slice replayability), Phase 4 NPC-05/NARR-01 amendments + Phase 3 SAVE-04 amendments queued for Phase 1→2 transition.
- **D-VISION-13:** **Any-to-any matchmaking with quality-based outcomes + telegraphed warnings + non-altruistic agency.** [Locked 2026-05-10 during slice matchmaking-pair Q5 reframing; sharpens D-VISION-04 matchmaking puzzle layer with quality-based outcomes specification.] **Core tenet:** ANY villager can be matched with ANY other villager (or with a rescued animal per D-VISION-11 — e.g., Cyrus × rescued-pet matched post-slice when sanctuary unlocks). The quality of the match depends on a per-pair **compatibility score** (chemistry / shared interests / personality fit / relationship-state-history). **Telegraphed warnings:** when player attempts a low-compatibility match, the game clearly surfaces the concern with explicit framing ("Hrm, are you sure? They don't really seem like a great fit because XYZ"). NO hidden penalties — Pillar 3 anti-ambiguous-puzzle honored. **Non-altruistic agency:** player CAN proceed with a poor match if they want; the relationship may become unhappy or fail, which is a player-chosen narrative outcome, not a game-imposed punishment. Pillar 2 (Never Punishing) honored — the player isn't punished, they're given full information and full agency to author the story they want. **Replayability via choice:** different matchmaking decisions across playthroughs produce different village outcomes; pairs naturally with D-VISION-09 NG+ time-loop (wisp recall: "the last time we matched Sprinkles with X, it didn't go well — try again with Y?"). **Phase 6 content load:** C(12,2) = 66 possible villager-pair compatibility profiles full game; slice handles C(4,2) = 6 pairs minus Cyrus = 3 active villager-villager pairs (Sprinkles × Ari, Sprinkles × Zhan, Ari × Zhan); plus Cyrus × rescued-pet post-slice content. Per-pair authored content scales: high-compatibility pairs get full meet-cute + happy arc; low-compatibility pairs get telegraph + attempt + unhappy outcome content. **Pillar compliance:** Pillar 1 (deep relationship surface; player agency in shaping matches), Pillar 2 (consequence not punishment; non-altruistic playthroughs preserved), Pillar 3 (telegraphed warnings, no hidden mechanics). Cross-references: SCOPE.md row 12 (2026-05-10), CONTEXT.md D-VISION-04 (this sharpens it), `09-slice-scenario-sketch.md` (plan 01-04 — slice authors at least 1 telegraphed-compatible pair as primary scenario; may hint at alternative attempt outcomes), `23-why-play-at-hour-200.md` (plan 01-06 — matchmaking-choice replayability as retention engine), Phase 4 NARR-01..06 + COMP-01..04 amendments queued for Phase 1→2 transition.
- **D-VISION-14:** **Tonal arc tracks player choices bidirectionally.** [Locked 2026-05-10 during World Bible §1 tone-summary authoring; sharpens D-VISION-13 tonal consequence dimension.] Sterling's tonal register evolves across the game's full arc as a meta-evolutionary gradient: **default trajectory (altruistic play)** = early game terse / sleepy / strife → late game warmth / coalition / cooperation. **Bidirectional response:** if the player consistently chooses non-altruistic options (poor matchmaking pursued despite telegraphed warnings per D-VISION-13; villager arcs left unfinished; cascading thread seeds ignored), the tonal arc remains in or regresses toward terse / sleepy / strife. The world **reflects** player choices, not **judges** them — Pillar 2 honored via "consequence, not punishment." Composes from existing locked architecture: NPC-05 state-tagged dialogue carries tonal register (warm / neutral / cool variants per line); D-VISION-10 cascading thread surfaces village changes; D-VISION-13 non-altruistic agency + telegraphed warnings; D-VISION-09 NG+ recall narration can reference prior playthrough's tonal outcome ("the last time, the village stayed cool — try something different?"). **Architectural implications:** every authored Phase 4 NARR line carries a tonal-state register tag (warm / neutral / cool variants); Phase 6 dialog authoring lands per-villager voice variants per tonal-state; Phase 6 art + audio palette can shift subtly per zone-tonal-state (cool-muted vs warm-saturated); SAVE-04 amendment records village tonal state per playthrough. **Pillar compliance:** Pillar 1 (player choices shape village state at scale — strong character development surface), Pillar 2 (consequence not punishment — non-altruistic playthroughs valid; world is responsive not judgmental), Pillar 3 (telegraphed via D-VISION-13 + NPC-05 state cues — no hidden tone shifts). Cross-references: SCOPE.md row 13 (2026-05-10), CONTEXT.md D-VISION-13 (this sharpens), `12-world-bible-v0.1.md` §1 tone summary, Phase 4 NPC-05 + NARR-01..06 amendments queued for Phase 1→2 transition (state-tagged dialog includes tonal-register tags).
